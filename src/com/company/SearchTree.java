...
public class SearchTree implements NodeList {
    ...
    public boolean addItem(ListItem newItem) {
        ...
        //otherwise, start comparing from te head of the tree
    ...
    private void performRemoval(ListItem item, ListItem parent) {
        ...
        if (item.next() == null) {
            ...
            if (parent.next() == item) {
                ...
                //parent must be the item itself, which means we are looking at the oor of the tree
            ...
            //neither left or right is null, so deletion is now trickier
            ...
            if (leftmostParent == item) {
                //there  was now leftmost node, so 'current' points to the smallest
                //node, the one that must be deleted
                item.setNext(current.next());
            } else {
                //set the smallest node's parent to point to
                //the smallest node's riight child, which may be null
